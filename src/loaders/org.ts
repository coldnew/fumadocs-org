...
export default function loader(
  ...
  if (callback) {
    ...
    try {
      // Create a synchronous version of org conversion for Turbopack
      // This skips async operations like file includes and validation for compatibility
      const { convertOrgToMdxSync } = require('../core/serialize');

      // Convert org to MDX synchronously (skips includes and complex validation)
      const conversionResult = convertOrgToMdxSync(source, filePath);
      return `${conversionResult.frontmatter}\n${conversionResult.markdown}`;
    } catch (error) {
      // If sync conversion fails, fall back to original source with warning
      console.warn(
        '[ORG] Synchronous org conversion failed, using original source:',
        error,
      );
      return source;
    }
